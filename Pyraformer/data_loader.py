...
class Dataset_Custom_2(Dataset):
    def __init__(
        self,
        root_path,
        flag="train",
...
class Dataset_Binance(Dataset):
    ...
    def __read_data__(self):
        ...
        #     7 * 30 * 24 * 6 + 4 * 30 * 24 * 6,
        # ]
        ### For 50% for Weather but 100% for Binance
        border1s = [
            0,
            7 * 30 * 24 * 3 - self.seq_len,
            7 * 30 * 24 * 3 + 2 * 30 * 24 * 3 - self.seq_len,
...
class Dataset_Custom2(Dataset):
    def __init__(
        ...
        self.all_data, self.covariates, self.train_end = eval("preprocess_flow")(
            preprocess_path
        )
    ...
    def __len__(self):
        if self.flag == "train":
            self.window_per_seq = (
                self.train_end - self.seq_len - self.pred_len
            ) // self.window_stride
            return self.window_per_seq * self.seq_num
        else:
            self.window_per_seq = (
                self.all_data.size(1) - self.test_start - self.seq_len - self.pred_len
            ) // self.window_stride
            return self.window_per_seq * self.seq_num
...
class Dataset_Synthetic(Dataset):
    def __init__(
        ...
        self.window_stride = 24
        window_per_seq = (
            self.all_data.shape[1] - self.seq_len - self.pred_len
        ) / self.window_stride
        self.train_end = (
            self.seq_len
            + self.pred_len
            + int(0.9 * window_per_seq) * self.window_stride
        )
        self.test_start = self.train_end - self.seq_len + 1
...
def preprocess_flow(csv_path):
    ...
    data_frame = pd.read_csv(
        csv_path, names=["app_name", "zone", "time", "value"], parse_dates=True
    )
    ...
    for i in range(len(grouped_data)):
        single_df = (
            grouped_data[i][1]
            .drop(labels=["app_name", "zone"], axis=1)
    ...
    all_data = (
        np.array([data[len(data) - min_length :, :] for data in all_data])
        .transpose(1, 0, 2)
        .astype(np.float32)
    )
...
class electTrainDataset(Dataset):
    def __init__(self, data_path, data_name, predict_length, batch_size):
        self.data = torch.from_numpy(
            np.load(os.path.join(data_path, f"train_data_{data_name}.npy"))
        )
        ...
        self.label = torch.from_numpy(
            np.load(os.path.join(data_path, f"train_label_{data_name}.npy"))
        )
